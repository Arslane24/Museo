...
class MuseumManager {
    ...
    public function addToFavorites($userId, $museumId) {
        try {
            $stmt = $this->pdo->prepare("INSERT INTO user_favorites (user_id, museum_id) VALUES (?, ?)");
    ...
    public function removeFromFavorites($userId, $museumId) {
        $stmt = $this->pdo->prepare("DELETE FROM user_favorites WHERE user_id = ? AND museum_id = ?");
        return $stmt->execute([$userId, $museumId]);
    }
    ...
    public function isFavorite($userId, $museumId) {
        $stmt = $this->pdo->prepare("SELECT id FROM user_favorites WHERE user_id = ? AND museum_id = ?");
    ...
    public function getUserFavorites($userId) {
        $sql = "SELECT m.* FROM museums m 
                INNER JOIN user_favorites f ON m.id = f.museum_id 
     ...
     */
    public function createBooking($userId, $museumId, $visitDate, $visitTime, $adults, $children, $totalPrice, $specialRequests = '') {
        $reference = 'MUSEOLINK-' . strtoupper(uniqid());
        
        $sql = "INSERT INTO bookings (user_id, museum_id, visit_date, visit_time, adults, children, total_price, special_requests, booking_reference, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed')";
        
        $stmt = $this->pdo->prepare($sql);
        $result = $stmt->execute([$userId, $museumId, $visitDate, $visitTime, $adults, $children, $totalPrice, $specialRequests, $reference]);
    ...
    public function getUserBookings($userId) {
        $sql = "SELECT b.*, m.name as museum_name, m.city, m.country, m.image_url 
                FROM bookings b 
                INNER JOIN museums m ON b.museum_id = m.id 
                WHERE b.user_id = ? 
