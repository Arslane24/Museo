...
// Ne pas afficher si l'utilisateur a déjà fait un choix (accepté OU refusé)
if (!empty($_COOKIE['cookie_consent'])) {
    if ($_COOKIE['cookie_consent'] === 'accepted' || $_COOKIE['cookie_consent'] === 'declined') {
        $showBanner = false;
    }
}

// Afficher la bannière uniquement si aucun choix n'a été fait
if ($showBanner): ?>
