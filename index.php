...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['accept_cookies'])) {
        // L'utilisateur accepte : créer le cookie de consentement
        $isSecure = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on';
        setcookie('cookie_consent', 'accepted', time() + (365 * 24 * 60 * 60), '/', '', $isSecure, true);
        // Ne PAS créer museo_theme ici - il sera créé quand l'utilisateur change le thème
        // Rediriger pour éviter la re-soumission du formulaire
        header('Location: ' . $_SERVER['REQUEST_URI']);
        exit;
    } elseif (isset($_POST['decline_cookies'])) {
        // L'utilisateur refuse : créer cookie de consentement persistant (30 jours)
        $isSecure = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on';
        setcookie('cookie_consent', 'declined', time() + (30 * 24 * 60 * 60), '/', '', $isSecure, true);
...
// Vérifier si l'utilisateur est connecté et charger l'état des favoris
$favoriteMuseums = [];
if (isset($_SESSION['user_id'])) {
    $data = require 'secret/database.php';
    $pdo = $data['pdo'];
    require_once 'src/models/MuseumManager.php';
    $museumManager = new MuseumManager($pdo);
    
    // Vérifier si les musées 1, 2, 3 sont en favoris
    $favoriteMuseums = [
        1 => $museumManager->isFavorite($_SESSION['user_id'], 1),
        2 => $museumManager->isFavorite($_SESSION['user_id'], 2),
        3 => $museumManager->isFavorite($_SESSION['user_id'], 3)
    ];
}
...
<section id="search" class="py-5" style="background: var(--bg-secondary); margin-top: 4rem;">
    <div class="container">
            ...
            <div class="col-md-4" data-aos="fade-up">
                <div class="museum-card" style="border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); transition: transform 0.3s ease, box-shadow 0.3s ease; background: white; border: none;">
                    <div style="position: relative; overflow: hidden; height: 280px;">
                        ...
                        <button class="favorite-btn <?= isset($favoriteMuseums[1]) && $favoriteMuseums[1] ? 'active' : '' ?>" data-museum-id="1" onclick="toggleFavorite(1, this)" style="position: absolute; top: 15px; left: 15px; width: 40px; height: 40px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); border: none; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: <?= isset($favoriteMuseums[1]) && $favoriteMuseums[1] ? '#ef4444' : '#9ca3af' ?>; font-size: 1.2rem; transition: all 0.3s ease; z-index: 10; cursor: pointer;">
            ...
            <div class="col-md-4" data-aos="fade-up" data-aos-delay="100">
                <div class="museum-card" style="border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); transition: transform 0.3s ease, box-shadow 0.3s ease; background: white; border: none;">
                    <div style="position: relative; overflow: hidden; height: 280px;">
                        ...
                        <button class="favorite-btn <?= isset($favoriteMuseums[2]) && $favoriteMuseums[2] ? 'active' : '' ?>" data-museum-id="2" onclick="toggleFavorite(2, this)" style="position: absolute; top: 15px; left: 15px; width: 40px; height: 40px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); border: none; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: <?= isset($favoriteMuseums[2]) && $favoriteMuseums[2] ? '#ef4444' : '#9ca3af' ?>; font-size: 1.2rem; transition: all 0.3s ease; z-index: 10; cursor: pointer;">
                            <i class="fas fa-heart"></i>
                        </button>
                        <div style="position: absolute; top: 15px; right: 15px; background: linear-gradient(135deg, #c9a961, #dfc480); color: white; padding: 0.4rem 0.9rem; border-radius: 50px; font-weight: 700; font-size: 0.85rem; box-shadow: 0 4px 10px rgba(0,0,0,0.2);">
            ...
            <div class="col-md-4" data-aos="fade-up" data-aos-delay="200">
                <div class="museum-card" style="border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); transition: transform 0.3s ease, box-shadow 0.3s ease; background: white; border: none;">
                    <div style="position: relative; overflow: hidden; height: 280px;">
                        ...
                        <button class="favorite-btn <?= isset($favoriteMuseums[3]) && $favoriteMuseums[3] ? 'active' : '' ?>" data-museum-id="3" onclick="toggleFavorite(3, this)" style="position: absolute; top: 15px; left: 15px; width: 40px; height: 40px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); border: none; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: <?= isset($favoriteMuseums[3]) && $favoriteMuseums[3] ? '#ef4444' : '#9ca3af' ?>; font-size: 1.2rem; transition: all 0.3s ease; z-index: 10; cursor: pointer;">
                            <i class="fas fa-heart"></i>
                        </button>
                        <div style="position: absolute; top: 15px; right: 15px; background: linear-gradient(135deg, #c9a961, #dfc480); color: white; padding: 0.4rem 0.9rem; border-radius: 50px; font-weight: 700; font-size: 0.85rem; box-shadow: 0 4px 10px rgba(0,0,0,0.2);">
                            <i class="fas fa-star me-1"></i>4.9
...
<script>
/**
 * Toggle favori - Ajouter/Retirer un musée des favoris
 */
async function toggleFavorite(museum_id, button) {
    try {
        const response = await fetch("api/favorites-toggle.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ museum_id: museum_id })
        });

        const data = await response.json();

        // Si non connecté, rediriger vers login
        if (data.error === "not_logged" || data.error === "You must be logged in") {
