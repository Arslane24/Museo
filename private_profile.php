...
$stmt = $pdo->prepare("SELECT id, name, login, email, created_at FROM users WHERE id = ?");
$stmt->execute([$userId]);
$user = $stmt->fetch();
...
// Date d'inscription réelle
$memberSince = $user['created_at'] ?? date('Y-m-d');

// Récupérer le musée favori (le plus récent dans la table favorites)
$favStmt = $pdo->prepare("
    SELECT m.name 
    FROM favorites f
    JOIN museums m ON f.museum_id = m.id
    WHERE f.user_id = ?
...
$favoriteMuseum = $favRow['name'] ?? 'Non renseigné';
...
$userInfo = [
    ...
    // plus de last_login fake
    'total_visits' => $stats['completed'],
    'favorite_museum' => $favoriteMuseum,
...
$badges = [
    ...
    [
        'name' => 'Passionné d\'Art',
        'icon' => 'palette',
        'color' => '#d4af37',
        'description' => 'Visite de 3 musées d\'art',
    ...
    [
        ...
        'icon' => 'gem',
        'color' => '#8b5cf6',
        'description' => '10+ favoris',
        'unlocked' => $stats['favorite_museums'] >= 10
    ],
    [
        'name' => 'Globe-trotter',
        'icon' => 'globe',
        'color' => '#06b6d4',
        'description' => 'Visité 5 villes',
        'unlocked' => $stats['cities_visited'] >= 5
    ],
];
...
.avatar-edit-btn {
    position: absolute;
    bottom: 35px;   /* décale légèrement pour être centré */
    right: 35px;    /* décale légèrement pour être centré */
    width: 40px;
    height: 40px;
    background: var(--gradient-accent);
    border-radius: 50%;
...
<div class="container py-4">
    ...
    <div class="profile-header">
        ...
        <div class="profile-info">
            ...
            <div class="profile-meta">
                ...
                <!-- Dernière connexion supprimée car pas de vraie donnée -->
            </div>
        </div>
    </div>
    ...
    <div class="stats-overview">
        ...
        <div class="mini-stat">
            ...
            <div class="mini-stat-value"><?= $stats['favorite_museums'] ?></div>
            <div class="mini-stat-label">Musées favoris</div>
        </div>
        <div class="mini-stat">
            <i class="fas fa-star"></i>
            <div class="mini-stat-value"><?= $unlockedBadgesCount ?></div>
            <div class="mini-stat-label">Badges débloqués</div>
        </div>
        <div class="mini-stat">
            <i class="fas fa-euro-sign"></i>
            <div class="mini-stat-value"><?= number_format($stats['total_spent'], 0) ?></div>
            <div class="mini-stat-label">Montant estimé dépensé (€)</div>
        </div>
    </div>
    ...
    <div class="private-card badges-section">
        <div class="section-header">
            <h2 class="section-title">
                ...
                Mes badges (<?= $unlockedBadgesCount ?>/<?= count($badges) ?>)
            </h2>
        </div>
            ...
            <div class="private-card">
                ...
                <div class="history-list">
                    <?php if (empty($visitHistory)): ?>
                        <p style="color: var(--gray-600);">Aucune visite pour le moment.</p>
                    <?php else: ?>
                        <?php foreach ($visitHistory as $visit): ?>
                            <div class="history-item">
                                ...
                                <div class="history-rating">
                                    <?php for($i = 0; $i < ($visit['rating'] ?? 0); $i++): ?>
            ...
            <div class="private-card">
                ...
                <div class="bio-section" style="margin-bottom: 1.5rem;">
                    <p style="color: var(--gray-600); line-height: 1.8;">
                        <?= $userInfo['bio'] !== '' ? htmlspecialchars($userInfo['bio']) : "Vous n'avez pas encore complété votre biographie." ?>
                    </p>
                </div>
                ...
                <div class="info-grid">
                    <div class="info-card">
                        ...
                        <div class="info-value">
                            <?= $userInfo['phone'] !== '' ? htmlspecialchars($userInfo['phone']) : 'Non renseigné' ?>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-value">
                            <?= ($userInfo['city'] !== '' || $userInfo['country'] !== '')
                                ? htmlspecialchars(trim($userInfo['city'].' '.$userInfo['country']))
                                : 'Non renseigné' ?>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-value">
                            <?= htmlspecialchars($userInfo['favorite_museum']) ?>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-value">
                            <?= htmlspecialchars($userInfo['preferred_language']) ?>
                        </div>
                    </div>
                </div>
            </div>
