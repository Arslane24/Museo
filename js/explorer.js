...
function initEventListeners() {
    ...
    if (searchInput) {
        ...
        console.error('searchInput non trouve');
    }
...
async function centerMapOnSearch(searchTerm) {
    if (!window.museumMap || !markersData || markersData.length === 0) {
        console.log('Carte ou markers non disponibles');
        return;
    }
    ...
    try {
        ...
        if (found) {
            ...
            console.log('Aucun musee trouve pour:', searchTerm);
            // Reinitialiser la vue pour montrer tous les markers
...
async function loadSpecificMuseum(museumId) {
    ...
    try {
        const url = `api/museums-search.php?id=${museumId}`;
        ...
        if (data.success && data.museum) {
            ...
            showError(data.error || 'Musee introuvable');
        }
    } catch (error) {
...
function createMuseumCard(museum) {
    ...
    const imageUrl = museum.image_url || museum.cover_image || '';
    const hasImage = !!imageUrl;
    const src = hasImage ? imageUrl : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
    
    // Vérifier que le slug existe
    if (!museum.slug) {
        console.warn('Musée sans slug:', museum.name, museum);
    }
    
    return `
        <div class="museum-card ${hasImage ? '' : 'no-image'}" data-aos="fade-up" data-museum-id="${museum.id}">
            <div class="museum-image">
                <img src="${src}" alt="${museum.name}"
                     onerror="this.style.display='none'; this.closest('.museum-card').classList.add('no-image')">
...
function attachCardEvents() {
    ...
    if (buttons.length === 0) {
        console.warn('Aucun bouton .btn-view trouvé dans le DOM');
...
function viewMuseum(slug) {
    console.log('Navigation vers musee:', slug);
...
function updateResultsInfo(pagination) {
    ...
    if (!titleEl || !countEl) {
        console.warn('Éléments resultsTitle ou resultsCount non trouvés');
        return;
    }
...
async function loadMetArtworks() {
    ...
    try {
        const artworks = await getMetMuseumArtworks('art', 6);
        
        if (artworks && artworks.length > 0) {
            grid.innerHTML = artworks.map(artwork => {
                const img = artwork.image || artwork.imageUrl || artwork.primaryImage || '';
                const hasImage = !!img;
                const src = hasImage ? img : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
                return `
                <div class="col-md-6 col-lg-4">
                    <div class="artwork-card ${hasImage ? '' : 'no-image'}">
                        <img src="${src}" 
                             alt="${artwork.title || 'Sans titre'}" 
                             class="artwork-card-img"
                             onerror="this.style.display='none'; this.closest('.artwork-card').classList.add('no-image')">
...
async function loadHarvard() {
    ...
    try {
        const artworks = await getHarvardArtworks('art', 3);
        
        if (artworks && artworks.length > 0) {
            grid.innerHTML = artworks.map(obj => {
                const img = obj.image || obj.imageUrl || obj.primary_image || obj.primaryimageurl || '';
                const hasImage = !!img;
                const src = hasImage ? img : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
                return `
                    <div class="col-md-6 col-lg-4">
                        <div class="artwork-card ${hasImage ? '' : 'no-image'}">
                            ...
                            <div class="artwork-card-body">
                                ...
                                <div class="artwork-card-artist">${obj.culture || obj.artist || 'Artiste inconnu'}</div>
                                ${obj.people && obj.people.length > 0 ? 
                                    `<div class="artwork-card-date">${obj.people[0].name || obj.date || ''}</div>` : ''}
                            </div>
                        </div>
                    </div>
                `;
    ...
    try {
        const promises = cities.map(city => 
            getWeather(city).catch(err => {
                console.error(`Erreur météo ${city}:`, err);
                return null;
            })
        );
        
        const results = await Promise.all(promises);
        const validWeather = results.filter(w => w !== null);
        
        if (validWeather.length > 0) {
            grid.innerHTML = validWeather.map(w => {
                return `
...
async function fetchSearchSuggestions(query) {
    try {
        console.log('📡 Appel API:', `api/museums-search.php?q=${query}`);
        const response = await fetch(`api/museums-search.php?q=${encodeURIComponent(query)}`);
        const data = await response.json();
        ...
        if (data.success && data.suggestions) {
            ...
            console.warn('Aucune suggestion');
            displaySuggestions([]);
        }
    } catch (error) {
...
function displaySuggestions(suggestions) {
    ...
    if (!container) {
        console.error('Container searchSuggestions non trouve');
        return;
    }
    ...
    try {
        ...
        if (data.success && data.museums) {
            ...
            if (museumMarkers.length > 0) {
                ...
                console.log('Vue ajustee pour afficher tous les musees');
            }
        } else {
