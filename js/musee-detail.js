...
async function loadArtworks() {
    ...
    try {
        // Appel a la fonction combinee avec le nom du musee
        const artworks = await getCombinedArtworks(museumData.name, 20);

        if (artworks && artworks.length > 0) {
            console.log(`${artworks.length} oeuvres recues`);
            ...
            // Pas d'oeuvres disponibles
            container.innerHTML = `
                <div class="col-12 text-center py-5">
                    <i class="fas fa-palette fa-3x mb-3" style="color: #64748b;"></i>
                    <p style="color: #94a3b8;">Aucune oeuvre disponible pour ce musee</p>
                </div>
            `;
        }
    } catch (error) {
