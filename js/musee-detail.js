...
function displayArtworksPage(page) {
    ...
    pageArtworks.forEach(artwork => {
        ...
        const imgHtml = artwork.hasImage 
            ? `<img src="${artwork.image}" alt="${titre}" onerror="this.src='https://via.placeholder.com/400x300/1a4d7a/ffffff?text=Image+non+disponible'">`
            : `<div style="height: 300px; background: linear-gradient(135deg, #1a4d7a 0%, #2c5f8d 100%); display: flex; align-items: center; justify-content: center; color: white; text-align: center; padding: 20px;">
                <div>
...
function showArtworkDetails(artwork) {
    ...
    const imgHtml = artwork.hasImage && artwork.image
        ? `<img src="${artwork.image}" alt="${artwork.title}" style="width: 100%; max-height: 400px; object-fit: contain; border-radius: 12px 12px 0 0;" onerror="this.src='https://via.placeholder.com/800x400/1a4d7a/ffffff?text=Image+non+disponible'">`
